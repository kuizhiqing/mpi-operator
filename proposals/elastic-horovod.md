...
However, the contemporary design of resilient-training-operator does not provide necessary information update to the launcher for elastic training. With the enhancement of resilient-training-operator to handle replicas change or pod failure, we are expecting to support Elastic Horovod in mpi-jobs.

## Goals
* Remove worker Pods when worker replicas is scaled less than the count of current worker Pods
* Provide a resilient-training-operator-specific `discover_hosts.sh` for [Elastic Horovod](https://docs.google.com/document/d/15ZoHA5AeSI_boeyIBapg9WPXKrYXMRvPytPzQWTCTn4/edit#) to discover available running workers.

## API
There are no API changes to ResilientJobs. Users who prefer non-elastic mode can simply ignore the `discover_hosts.sh` script.
...
1. The ResilientJob Controller listens for pods, listing all worker pods with running phase
...
5. The ResilientJob Controller updates the rule in RBAC resources as well, enabling launcher pod to launch new worker process on newly created worker pods
