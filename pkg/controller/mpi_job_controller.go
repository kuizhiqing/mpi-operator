...
const (
	...
	elasticLableName  = "kubeflow.com/elastic"
	launcherAsWorker  = "kubeflow.com/launcher-as-worker"
...
func (c *MPIJobController) syncHandler(key string) error {
	...
	if !done {
		...
		if config, err := c.getOrCreateLaunchConfigMap(mpiJob); config == nil || err != nil {
			return fmt.Errorf("creating launch: %w", err)
		}
...
func (c *MPIJobController) cleanFailedPods(mpiJob *kubeflow.MPIJob, launcher *corev1.Pod, worker []*corev1.Pod) error {
	...
	// no delete if launcher not running
...
func noRestartError(cs *corev1.ContainerStatus) bool {
	if cs.State.Terminated != nil {
		...
		if cs.State.Terminated.Reason == "ContainerCannotRun" {
			return true
		}
	...
	if lastState.Terminated != nil {
		...
		if lastState.Terminated.Reason == "ContainerCannotRun" {
			return true
		}
...
func backoffLimitExceeded(pod *corev1.Pod) bool {
	...
	if restarts < 1 {
		return false
	} else if total < time.Duration(1*time.Hour) && restarts >= 2 {
		klog.Info(fmt.Sprintf("backoffLimitExceeded %s, restarts %d, %v", pod.Name, restarts, cstatus.State))
		return true
	} else if total < time.Duration(24*time.Hour) && restarts >= 4 {
		klog.Info(fmt.Sprintf("backoffLimitExceeded %s, restarts %d, %v", pod.Name, restarts, cstatus.State))
		return true
	} else if restarts >= 8 {
		klog.Info(fmt.Sprintf("backoffLimitExceeded %s, restarts %d, %v", pod.Name, restarts, cstatus.State))
...
func (c *MPIJobController) syncLauncherImage(mpiJob *kubeflow.MPIJob, launcher *corev1.Pod) error {
	if launcher != nil {
		...
		if launcherImg != podImg && isPodRunning(launcher) {
			// reset pod image
...
func (c *MPIJobController) updateMPIJobStatus(mpiJob *kubeflow.MPIJob, launcher *corev1.Pod, worker []*corev1.Pod, horker []*corev1.Pod) error {
	...
	for i := 0; i < len(horker); i++ {
		...
		case corev1.PodFailed:
			...
			if horker[i].Status.Reason == "Evicted" {
				evict += 1
			}
		case corev1.PodSucceeded:
	...
	if evict > 0 {
		msg := fmt.Sprintf("%d pods are evicted", evict)
	...
	if mpiJob.Status.CompletionTime == nil {
		...
		} else if launcher != nil && allContainerRunning(launcher, worker, horker) {
...
func (c *MPIJobController) newReplicas(mpiJob *kubeflow.MPIJob, index int, rtype kubeflow.MPIReplicaType) *corev1.Pod {
	...
	c.setupLaunchOnPod(&podTemplate.Spec, mpiJob)
