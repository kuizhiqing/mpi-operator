...
const (
	...
	controllerName               = "resilient-training-operator"
)
...
var (
	isLeader = promauto.NewGauge(prometheus.GaugeOpts{
		...
		Help: "Is this client the leader of this resilient-training-operator client set?",
	})
)
...
func Run(opt *options.ServerOption) error {
	...
	run := func(ctx context.Context) {
		...
		controller := controllersv1.NewResilientJobController(
			kubeClient,
			mpiJobClientSet,
			volcanoClientSet,
			schedClientSet,
	...
	election.RunOrDie(ctx, election.LeaderElectionConfig{
		...
		Name:     "resilient-training-operator",
...
func createClientSets(
	...
	kubeClientSet, err := kubeclientset.NewForConfig(restclientset.AddUserAgent(config, "resilient-training-operator"))
	if err != nil {
